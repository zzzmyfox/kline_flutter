...
class _KlineViewState extends State<KlineView> {
  ...
  Future addSingleData() async {
    ...
    _endDataList
        .addAll(_totalDataList.sublist(startIndex, _totalDataList.length));
  ...
  void _getClickData(double clickX) {
    if (_isShowDetail) {
      ...
      for (int i = 0; i < _viewDataList.length; i++) {
        if (_viewDataList[i].leftStartX <= clickX &&
            _viewDataList[i].rightEndX >= clickX) {
          _lastData = _viewDataList[i];
          _detailDataList
              .add(_chartUtils.dateFormat(_lastData.timestamp, year: true));
          ...
          String upDownRate = _chartUtils.setPrecision(
              upDownAmount / _lastData.openPrice * 100, 2);
          if (upDownAmount > 0) {
  ...
  void _onTapDown(TapDownDetails details) {
    ...
    if (_viewDataList[0].leftStartX <= moveX &&
        _viewDataList[_viewDataList.length - 1].rightEndX >= moveX) {
      setState(() {
        _isShowDetail = true;
        _getClickData(moveX);
      });
